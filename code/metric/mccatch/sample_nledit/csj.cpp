...
public:
    ...
    void simJoin(mySlimTree *T, mySlimTree *_T, double range, vector<unordered_set<TName* >> E)
    {
...
private:
    ...
    void mergeIntoPrevGroup(TName* p, TName* q, vector<unordered_set<TName *>> E)
    {
        ...
        for(; i<sizeGroups; i++)
        {
            std::unordered_set<TName*> Ej = E[i];
    ...
    void countLeafs(u_int32_t pageID, std::unordered_set<TName*> &leafs, mySlimTree *SlimTree)
    {
